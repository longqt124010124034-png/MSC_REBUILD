...
import { Users, BookOpen, ChevronRight, GraduationCap, Briefcase, Sparkles, LucideIcon } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";

// Định nghĩa kiểu cho các tab để đảm bảo type-safety
type Tab = 'faculty' | 'methods' | 'successors';

// Định nghĩa kiểu cho một điểm trong phương pháp giảng huấn
type TeachingPoint = {
  icon: LucideIcon; // Kiểu dữ liệu cho một component icon từ lucide-react
  title: string;
  text: string;
};

export default function MentorsPage() {
  // Sử dụng kiểu `Tab` đã định nghĩa để đảm bảo an toàn kiểu cho state
  const [activeTab, setActiveTab] = useState<Tab>('faculty');

  // Dữ liệu ban giảng huấn
  const faculty = [
    {
      id: "phan-huynh-anh",
      name: "Phan Huỳnh Anh",
  ...
  // Dữ liệu phương pháp giảng huấn với kiểu dữ liệu rõ ràng
  const teachingMethods: { title: string; description: string; points: TeachingPoint[] } = {
                          ...
                          <motion.div key={index} variants={itemVariants}>
                              <Card className="p-8 flex items-start space-x-6 bg-white shadow-lg rounded-2xl">
                                  <div className="flex-shrink-0 w-16 h-16 bg-gradient-to-br from-blue-500 to-teal-400 rounded-xl flex items-center justify-center">
                                      {/* TypeScript giờ đây biết point.icon là một component React hợp lệ */}
